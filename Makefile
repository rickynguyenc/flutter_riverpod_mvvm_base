...
.PHONY: build-ios-dev
build-ios-dev:
	fvm flutter build ios --no-codesign development --dart-define=FLAVOR=development --target lib/main.dart

.PHONY: build-ios-prd
build-ios-prd:
	fvm flutter build ios --release --no-codesign production --dart-define=FLAVOR=production --target lib/main.dart
